...
import { Button, ConfirmDialog, Dialog, SpecViewer, VersionHistoryPanel } from "@bismo/ui";
import { ApiError } from "@/lib/api-client";
import { useDeleteSpecification, useSpecificationVersions, useSpecifications } from "../queries";
...
export function SpecificationPanel({
  ...
  const deleteSpec = useDeleteSpecification(
    selectedSpec ? { parentType: selectedSpec.parentType, parentId: selectedSpec.parentId } : { parentType, parentId },
  );
  const { data: versionsData, isLoading: versionsLoading } = useSpecificationVersions(
    historyOpen ? selectedSpecId : null,
  );
      ...
      <div className="flex flex-wrap gap-2">
        {specs.map((spec) => (
          <button
            ...
            style={{
              borderColor: spec.id === selectedSpecId ? accentColor : "var(--bismo-border)",
              color: spec.id === selectedSpecId ? "var(--bismo-text)" : "var(--bismo-text-muted)",
            }}
          >
            {spec.filename}
          </button>
