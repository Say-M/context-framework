...
async function tryRefresh(): Promise<boolean> {
  if (!refreshInFlight) {
    refreshInFlight = (async () => {
      try {
        const res = await rawFetch("/api/v1/platform-auth/refresh", { method: "POST" });
...
export async function apiFetch(path: string, init: RequestInit = {}) {
  ...
  if (res.status === 401 && !path.startsWith("/api/v1/platform-auth/")) {
...
/** Fetches a binary response as a blob and triggers a browser download — there's no JSON body to parse. */
